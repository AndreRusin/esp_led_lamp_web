...
void changePower() {
  ...
  if (ONflag) {
    effectsTick();
    for (uint8_t i = 0U; i < modes[currentMode].bright; i = constrain(i + 8, 0, modes[currentMode].bright)) {
    ...
    effectsTick();
    for (uint8_t i = modes[currentMode].bright; i > 0; i = constrain(i - 8, 0, modes[currentMode].bright)) {
      FastLED.setBrightness(i);
      delay(1);
      FastLED.show();
    }
